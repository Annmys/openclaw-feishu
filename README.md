...
- 🔧 **解决飞书 API 调用次数耗尽问题**
  - 给 `probe.ts` 增加缓存机制
  - 缓存时间 24 小时，避免频繁调用 `/open-apis/bot/v3/info`
  - 添加 `clearProbeCache()` 函数用于测试和凭证变更时清除缓存
  - 解决每分钟 API 调用导致额度耗尽的问题

### v0.2.6 (2026-02-07)
- 🐛 **修复模型分配功能完整性**
  - 修复 bot.ts 中未传递模型分配参数的问题
  - 确保动态 Agent 创建时正确应用模型配置
  - 完善整体功能逻辑和协调性

### v0.2.5 (2026-02-07)
- ✨ **新增模型分配模式切换**
  - `unified` (统一模式)：所有用户使用相同默认模型
  - `assigned` (分配模式)：按用户配置分配不同模型（默认）
  - 支持通过配置灵活切换模式
  - 更新 INSTALL.md 配置文档
...
- ✅ 通过 TypeScript 类型检查和构建测试

### v0.2.0 (2026-02-07)
- 🎉 初始版本发布
- ✨ 中央权限管理（L1/L2/L3/L0 四层权限）
- 🤖 动态 Agent 创建与主控监控
- 👤 智能用户身份识别
- 📊 任务汇报与审计日志
